...
Roughly implemented today: authentication (incl. Google on mobile), user profile, setup wizard, **tasks**, **pathways**, **invitations**, **careGroup notes**, **care group members** list, home shell.  
...
- **First-run / returning user** flows: loading profile, resuming wizard, “needs careGroup” empty states.
- **Account lifecycle**: prefer disable Auth + soft-delete over hard delete from the client (matrix).

---

## 2. CareGroups, care group, and recipients

- **CareGroup** create/update: name, description, `pathwayIds`, `recipientIds` / **recipient profiles**; **`principal_carer`** for settings updates; `careGroupId` not rewritable from untrusted client.
- **Edit careGroup** UX: who sees what (carers read-only summary vs principal edits) per brief.
...
- **CareGroup / care group discovery** for invitees (already partly implied by rules).

---

## 4. Care pathways (extend current)

- **System pathways** (`system == true`): browse and attach to careGroup `pathwayIds`.
...
- Full **CRUD** and filters per careGroup; **soft-delete / archive** and who can transition (open matrix item).
...
- **CareGroup journal** (`journalEntries`): timeline of entries; create/update by **principal** + **carer**; who sees what — **brief implies carers see feed**; **limited recipient** “own only vs all” **TBD** (matrix).

---

## 8. Calendar and scheduling

**Goal:** One place to plan care-related time — distinct from, but can **link to**, the **careGroup meetings** subcollection in the matrix.
...
- **Meetings** (`careGroups/.../meetings`): multi-party **care coordination meetings** (agenda, notes, follow-ups) — may surface as calendar events or a dedicated list; rules already assume principal+carer create; refine copy.
...
- **CareGroup documents** with categories (e.g. **medical**, **other**, **legal**, **financial**); **sensitiveRoles**-style gating: legal/financial visibility for **principal**, **POA**, **financial_manager** as product defines.
...
- **Shared contact directory** for the careGroup (CCGs, GPs, trades, family): CRUD for principal + carer; recipient visibility **conditional** (matrix).
...
## 14. Chat (care group or careGroup)

- **Group messaging** (recommended place in matrix: `careGroups/{careGroupId}/messages` or under careGroup): at least **principal**, **carer**, **financial**, **POA**; **receives_care** rules are conservative in `firestore.rules` — product may expand.
...
- **POA / sensitive action audit** (`careGroups/{hid}/auditLogs/...` append-only, actor = self or Fn-only).
...
- **Custom claims** vs **member doc** reads: performance strategy (bundle, replication of `careGroupIds` on `users` via **Fn/Admin** only).
